...
#sequence_length = 10  # 입력길이
batch_size = 5  # 배치사이즈

# 입력값
X = tf.placeholder(tf.int32, [batch_size, None])  # X one-hot, None : batch_size
Y = tf.placeholder(tf.int32, [batch_size, None])  # Y label, None : batch_size
Lengths = tf.placeholder(tf.int32, [batch_size])  #length array
Max_length = tf.placeholder(tf.int32, None)
...
initial_state = cell.zero_state(batch_size, tf.float32)  # 초기 스테이트
...
outputs = tf.reshape(outputs,[batch_size,Max_length,hidden_size])  #하나로 합친것을 다시 펼친다

# Cost
weights = tf.ones([batch_size, Max_length])
...
for epoch in range(1):
    for i in range(0,len(csv_list),batch_size):
        ...
        for j in range(1):
            sess.run(train, feed_dict={X: x_batch, Y: y_batch, Lengths: lengths, Max_length:max_length})
            y_result = sess.run(prediction, feed_dict={X: x_batch, Lengths:lengths, Max_length:max_length})
